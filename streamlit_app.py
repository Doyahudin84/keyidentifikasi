...
def kunci_determinan():
    ...
    biji = st.radio("Apakah tumbuhan berbiji?", ["Ya", "Tidak"])
    
    if biji == 'Ya':
        # Jika tumbuhan berbiji, tentukan apakah bijinya tertutup atau terbuka
        biji_tertutup = st.radio("Apakah biji tertutup dalam buah?", ["Ya", "Tidak"])
        
        if biji_tertutup == 'Ya':
            # Angiospermae (Tumbuhan Berbiji Tertutup)
            biji_keping = st.radio("Apakah biji berkeping satu (monokotil) atau dua (dikotil)?", ["Satu", "Dua"])
        ...
        jenis_tumbuhan = st.radio("Apakah tumbuhan ini memiliki akar, batang, dan daun sejati?", ["Ya", "Tidak"])
